...
int hook_direct(struct hook_t *h, unsigned int addr, void *hookf) {
  ...
  h->orig = addr;
  log("orig = %x\n", h->orig) h->jump[0] = 0xe59ff000; // LDR pc, [pc, #0]
  h->jump[1] = h->patch;
  h->jump[2] = h->patch;
  for (i = 0; i < 3; i++)
  ...
  hook_cacheflush((unsigned int)h->orig,
                  (unsigned int)h->orig + sizeof(h->jumpt));
  return 1;
}
...
int hook(struct hook_t *h, int pid, char *libname, char *funcname,
  ...
  if (addr % 4 == 0) {
    log("ARM using 0x%lx\n", (unsigned long)hook_arm) h->thumb = 0;
    ...
    if ((unsigned long int)hook_thumb % 4 == 0)
      log("warning hook is not thumb 0x%lx\n", (unsigned long)hook_thumb)
          h->thumb = 1;
    log("THUMB using 0x%lx\n", (unsigned long)hook_thumb) h->patch =
