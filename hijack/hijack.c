...
static int find_libc(char *name, int len, unsigned long *start, struct mm *mm,
                     int nmm) {
  int i;
  struct mm *m;
  char *p;
...
static int write_mem(pid_t pid, unsigned long *buf, int nlong,
                     unsigned long pos) {
  unsigned long *p;
  int i;
...
static int read_mem(pid_t pid, unsigned long *buf, int nlong,
                    unsigned long pos) {
  unsigned long *p;
  int i;

  for (p = buf, i = 0; i < nlong; p++, i++)
    if ((*p = ptrace(PTRACE_PEEKTEXT, pid, (void *)(pos + (i * 4)),
                     (void *)*p)) < 0)
      return -1;
  return 0;
}
...
#define HELPSTR                                                                \
  "error usage: %s -p PID -l LIBNAME [-d (debug on)] [-z (zygote)] [-m (no "   \
  "mprotect)] [-s (appname)] [-Z (trace count)] [-D (debug level)]\n"
...
int main(int argc, char *argv[]) {
  ...
  if (appname) {
    if (ptrace(PTRACE_SETOPTIONS, pid, (void *)1,
  ...
  */
  ...
  if (0 > write_mem(pid, (unsigned long *)&sc, sizeof(sc) / sizeof(long),
                    codeaddr)) {
